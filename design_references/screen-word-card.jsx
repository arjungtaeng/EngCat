...
function ECScreenWordCard() {
  ...
  // review / preview 모드 — 단어 다 끝나도 표현이 있으면 sentence-card로 이어짐
  const sourceMode = cardSource.current && cardSource.current.mode;
  const isReviewOrPreview = sourceMode === 'review' || sourceMode === 'preview';
  const sourceExpressions = (cardSource.current && cardSource.current.expressions) || [];
  const hasExpressions = sourceExpressions.length > 0;
    ...
    if (dir === 'next') {
      ...
      if (isLast) {
        ...
        if (isReviewOrPreview) {
          if (hasExpressions) {
            // 단어 → 표현 연결. ECCardSource 갱신해 sentence-card에서도 접근 가능.
            window.ECCardSource = {
              mode: sourceMode,
              words: cardSource.current.words,
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* Word + speaker — 줄바꿈 감지: 짧으면 옆에, 길면 마지막 글자 바로 아래 */}
          <WordWithSpeaker word={word} speak={speak} T={T} isDark={isDark} />
