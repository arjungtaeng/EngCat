            ...
            {savedWords.map((w) => (
              <div key={w.id} style={{
                ...
                >
                  {/* 프로필 아바타처럼 왼쪽 원형 썸네일 */}
                  {w.img
                    ? <img src={w.img} alt={w.en} style={{ width: 85, height: 85, borderRadius: 12, objectFit: 'cover', flexShrink: 0, border: `1px solid ${T.hairStr}` }} />
                    : <ECPlaceholder height={85} tint={w.tint} radius={12} style={{ width: 52, flexShrink: 0 }} />
                  }
          ...
          <div style={{ padding: '16px 18px 0', display: 'flex', flexDirection: 'column', gap: 10 }}>
            {savedSentences.map((s) => (
              ...
              >
                {/* 프로필 아바타처럼 왼쪽 원형 썸네일 */}
                {s.img
                  ? <img src={s.img} alt={s.en} style={{ width: 85, height: 85, borderRadius: 12, objectFit: 'cover', flexShrink: 0, border: `1px solid ${T.hairStr}` }} />
                  : <ECPlaceholder height={85} tint={s.tint} radius={12} style={{ width: 52, flexShrink: 0 }} />
                }
                <div style={{ flex: 1, minWidth: 0 }}>
