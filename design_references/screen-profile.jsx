...
function ECScreenProfile() {
  ...
  return (
    <div style={{ flex: 1, minHeight: 0, background: T.bg1, display: 'flex', flexDirection: 'column' }}>
      <div ref={scrollRef} style={{ flex: 1, overflowY: 'auto', WebkitOverflowScrolling: 'touch', paddingBottom: 20 }}>
      <ECStatusBar />
      ...
      <div style={{ padding: '20px 18px 0' }}>
        <div style={{
          ...
          {user?.picture
            ? <img src={user.picture} alt={user.name} referrerPolicy="no-referrer" style={{ width: 76, height: 76, borderRadius: 999, objectFit: 'cover', border: `1px solid ${T.hairStr}` }} />
            : <div style={{
                width: 76, height: 76, borderRadius: 999,
                background: `linear-gradient(135deg, ${T.art1} 0%, ${T.art2} 100%)`,
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontFamily: T.serif, fontSize: 27, color: T.text, fontStyle: 'italic',
          ...
          <svg viewBox="0 0 80 80" style={{ width: 80, height: 80, flexShrink: 0 }}>
            ...
            />
            {/* 중앙 텍스트 */}
            <text x="40" y="37" textAnchor="middle" dominantBaseline="middle" fontSize="20" fontWeight="600" fill={T.text} fontFamily={T.serif}>
              B2
            </text>
            <text x="40" y="52" textAnchor="middle" dominantBaseline="middle" fontSize="10" fill={T.textMute} fontFamily={T.mono}>
              47개
            </text>
          </svg>
        </div>
